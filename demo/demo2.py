...
os.chdir('D:\\PythonProject\\Test\\test1\\data\\input\\',)# 改变当前工作目录。注意路径的分隔符
...
for txt in fileTxt_list:
    ...
    resultTemp_matrix = np.zeros([6, 304], float)  # 创建一个6行，304列的二维数组（矩阵）(304是所有数据统计下来的单一周期的最大值)
...
# print(result_matrix)
# 以数组形式，保存到txt中
np.savetxt('D:\\PythonProject\\Test\\test1\\data\\output\\output.csv', result_matrix, delimiter=",")
b = np.loadtxt('D:\\PythonProject\\Test\\test1\\data\\output\\output.csv', delimiter=",")
print(b)
