...
os.chdir('../data/input/')# 改变当前工作目录。注意路径的分隔符
...
for txt in fileTxt_list:
    ...
    max_index, max_value = ss.find_peaks(data[:, 0], height= 0.06, distance=200)# 识别峰值，设置限制，高度为0.06，距离为200  max_index为最高点下标，max_value为最高点的值
...
np.savetxt('../output/output.csv', result_matrix, delimiter=",")# 将数组存储到output中
b = np.loadtxt('../output/output.csv', delimiter=",") # 读取数组（读得数据为数组形式）
print(b)
