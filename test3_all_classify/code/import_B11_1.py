...
''' 数据预处理  生成标准CSV格式表格数据 ，含1000个特征，一个未编码标签 
    ...
    不良品 B11_1 数据，标签为 9 （缺少B7，去除B8，所以标签前移）
        此为 B11类型故障的第一种类型，值较低
    '''
import numpy as np
import pandas as pd
import os
...
os.chdir('../data/input_B11_1/')  # 改变当前工作目录。注意路径的分隔符
...
for txt in fileTxt_list:
    ...
    max_index, max_value = ss.find_peaks(data[:, 0], height=0.18, distance=200)  # 识别峰值，设置限制，高度为，距离为200  max_index为最高点下标，max_value为最高点的值
...
result_matrix[:, 1000] = 9  # 写标签

# print(result_matrix)

np.savetxt('../output_B11_1/output.csv', result_matrix, delimiter=",")  # 将数组存储到output中
b = np.loadtxt('../output_B11_1/output.csv', delimiter=",")  # 读取数组（读得数据为数组形式）
print(b)
