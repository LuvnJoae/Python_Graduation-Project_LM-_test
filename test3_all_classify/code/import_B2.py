...
os.chdir('../data/input_B2/')  # 改变当前工作目录。注意路径的分隔符
...
for txt in fileTxt_list:
    ...
    # max_index, max_value = ss.find_peaks(data[:, 0], height=0.002, distance=200)  # 识别峰值，设置限制，高度为，距离为200  max_index为最高点下标，max_value为最高点的值
    # time_index = max_index * 0.000005  # max_index是从1开始的下标,所以乘上一个时间间隔，得到实际时间下标

    temp = data[0: 1000].T  # 从第一个峰值开始，往后数1000个数据，将这些数据给result_matrix矩阵中
...
result_matrix[:, 1000] = 2  # 写标签

# print(result_matrix)

np.savetxt('../output_B2/output.csv', result_matrix, delimiter=",")  # 将数组存储到output中
b = np.loadtxt('../output_B2/output.csv', delimiter=",")  # 读取数组（读得数据为数组形式）
print(b)
