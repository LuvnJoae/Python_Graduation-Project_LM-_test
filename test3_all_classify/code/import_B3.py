...
os.chdir('../data/input_B3/')  # 改变当前工作目录。注意路径的分隔符
...
for txt in fileTxt_list:
    ...
    max_index, max_value = ss.find_peaks(data[:, 0], height=0.08, distance=200)  # 识别峰值，设置限制，高度为，距离为200  max_index为最高点下标，max_value为最高点的值
...
result_matrix[:, 1000] = 3  # 写标签

# print(result_matrix)

np.savetxt('../output_B3/output.csv', result_matrix, delimiter=",")  # 将数组存储到output中
b = np.loadtxt('../output_B3/output.csv', delimiter=",")  # 读取数组（读得数据为数组形式）
print(b)
