...
''' 数据预处理  生成标准CSV格式表格数据 ，含1000个特征，一个未编码标签 
    ...
    '''
''' 注意 
        对于B6，与良品波形相比，没有明显的特征，而且给的数据也不统一，所以不明白这个电流增大，是指某一个周期值变高，还是整体值上台（这两种情况在B6的数据里都有体现）
        也正因此，在训练神经网络中，系统会将B6与良品波形混为一体，
        此处，为了使系统更加完整与准确，暂定电流增大导致的现象为电流值整体上台。
        在原数据的基础上，加上0.05的值。
'''
import numpy as np
import pandas as pd
...
os.chdir('../data/input_B6/')  # 改变当前工作目录。注意路径的分隔符
...
for txt in fileTxt_list:
    ...
    max_index, max_value = ss.find_peaks(data[:, 0], height=0.088, distance=200)  # 识别峰值，设置限制，高度为，距离为200  max_index为最高点下标，max_value为最高点的值
...
result_matrix[:, 1000] = 6  # 写标签

# print(result_matrix)

np.savetxt('../output_B6/output.csv', result_matrix, delimiter=",")  # 将数组存储到output中
b = np.loadtxt('../output_B6/output.csv', delimiter=",")  # 读取数组（读得数据为数组形式）
print(b)
