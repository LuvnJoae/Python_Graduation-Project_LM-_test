...
''' 注意
        # 这个程序是用来生成 测试 文件的，可重复使用 ，用于生成无周期的数据文件
        # 更换测试案例时，要修改的地方有：
        ...
        # 以不含周期的方式 生成
        由于B2的波形是直线，所以不采用周期化，而是直接从0开始，往后1000个数据
'''
...
os.chdir('../data/test/B5/')  # 改变当前工作目录。注意路径的分隔符
...
for txt in fileTxt_list:
    ...
    # max_index, max_value = ss.find_peaks(data[:, 0], height=0.002, distance=200)  # 识别峰值，设置限制，高度为0.08，距离为200  max_index为最高点下标，max_value为最高点的值
